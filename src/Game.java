...
class GLUtil {
  ...
  static void vertexAttribArrayUnbind(int id) {
    ...
    vboBind(vboId);
		...
		GL20.glVertexAttribPointer(attrId, 3, GL11.GL_FLOAT, false, 0, 0); // HARDCODED !
    vboUnbind();
	}
}
...
public class Game {
    ...
    // Geometry startup
		Model model = modelMake(Data.vertices);
	...
	static Model modelMake(float[] positions) {
    ...
    GLUtil.vertexAttribArrayBind(K.attr0); // CLEANUP: hardcoded ! put this into the model instead
		GL11.glDrawArrays(GL11.GL_TRIANGLES, K.offset0, model.vertexCount);
